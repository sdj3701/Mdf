...
Verified by: see Verification section below; migrated from old Status: verified-from-repo; Mdfproject; artifacts/singleplayer-breakwall-addressables-latest.log; artifacts/singleplayer-breakwall-addressables-latest.log; unity-cli --project Mdfproject status => ready; editor refresh --compile => Compilation complete; console errors => []; unity-cli --project Mdfproject status => ready; editor refresh --compile => Compilation complete; console --type error => []; unity-cli --project Mdfproject status => ready; editor refresh --compile => Compilation complete; console --type error => []
Replacement: none
