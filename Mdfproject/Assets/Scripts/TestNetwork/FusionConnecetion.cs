...
// 기존 스크립트와 구분을 위해 클래스 이름을 변경했습니다.
public class FusionConnecetion : MonoBehaviour, INetworkRunnerCallbacks
{
    [Header("Network")]
    // 게임 세션을 관리하는 핵심 컴포넌트입니다.
    ...
    private enum ConnectionState { Disconnected, InLobby, InGame }
    ...
    private async void StartGame(GameMode mode, string sessionName)
    {
        ...
        Debug.Log($"Starting Game with session name: {sessionName}");

        // 로비에 있을 때 사용하던 Runner를 그대로 사용하여 게임을 시작합니다.
        ...
        switch (_state)
        {
            ...
            case ConnectionState.InLobby:
                ...
                if (GUI.Button(new Rect(10, 90, 200, 50), "Create Room"))
                {
                    ...
                    StartGame(GameMode.Host, _roomNameInput);
                }
