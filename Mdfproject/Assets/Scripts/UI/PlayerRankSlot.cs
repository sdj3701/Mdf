 ...
 public class PlayerRankSlot : MonoBehaviour
 {
     ...
     public void Initialize(PlayerManager player)
     {
         ...
         {
             // UI 요소들의 참조가 유효한지 확인
             // PlayerManager가 스폰된 상태인지 확인하고, 그렇지 않은 경우 기본값 사용
             string playerIdString = trackedPlayer.HasStateAuthority ? trackedPlayer.playerId.ToString() : "Unknown";
             string displayName = $"Player {playerIdString}";
             if (playerNameText != null)
             {
